...
def find_latest_app(resp_json, platfrom):
    ...
    for item in resp_json["items"]:
        if (
            latest_app == None
            or item["upload_timestamp"] > latest_app["upload_timestamp"]
        ):
            latest_app = item
    return latest_app
...
resp = requests.get(
    f"https://api.{sl_region}.saucelabs.com/v1/storage/files", auth=(sl_user, sl_key)
)
resp_json = resp.json()
...
if resp.status_code == 200:
    ...
    raise Exception(
        f"There was a problem getting the list of available apps in Sauce Labs: {resp_json}"
    )
