...
def agent_controller_GET(url, topic, operation=None, id=None) -> (int, str):
    agent_url = url + topic
    if operation:
...
def agent_controller_DELETE(url, topic, id=None, data=None) -> (int, str):
    ...
    if id:
        agent_url = agent_url + id
    (resp_status, resp_text) = run_coroutine_with_kwargs(make_agent_controller_request, "DELETE", agent_url)
    return (resp_status, resp_text)
...
def check_if_already_connected(context, sender, receiver):
    ...
    (resp_status, resp_text) = agent_controller_GET(receiver_url + "/agent/command/", "did")
    ...
    if resp_status == 200:
        ...
        (sender_resp_status, sender_resp_text) = agent_controller_GET(sender_url + "/agent/command/", "active-connection", id=receiver_did)
        if sender_resp_status == 200:
            ...
            (resp_status, resp_text) = agent_controller_GET(receiver_url + "/agent/command/", "active-connection", id=sender_did)
            if resp_status == 200:
                resp_json = json.loads(resp_text)
...
def setup_already_connected(context, requester_connection_info_json, requester, responder):
    ...
    (resp_status, resp_text) = agent_controller_GET(responder_url + "/agent/command/", "active-connection", id=requester_did)
    if resp_status == 200:
        resp_json = json.loads(resp_text)
