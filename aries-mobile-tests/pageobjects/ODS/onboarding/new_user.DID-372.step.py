...
def stepGivenNewUser2And1(context) -> None:
    context.initialOnboarding = initialOnboarding(context)
    assert context.initialOnboarding.rightPage() ==True, "You are not on the Get Started Page"
    

@when("I click on Get started")
def stepWhenNewUser2(context) -> None:
    assert context.initialOnboarding.selectGetStartedBtn() ==True, "Can't find or click the Get started Btn"
...
def stepThenNewUser2(context) -> None:
    ...
    assert context.touchIDBiometrics == True, "You are not on the biometric page"
