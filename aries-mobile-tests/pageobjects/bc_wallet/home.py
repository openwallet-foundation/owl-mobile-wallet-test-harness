...
class HomePage(BasePage):
    ...
    on_this_page_locator = (AppiumBy.NAME, "Notifications")
    ...
    view_credential_offer_notification_button_locator = (
        AppiumBy.ID,
        "com.ariesbifold:id/ViewOffer",
    )
    view_proof_notification_button_locator = (
        AppiumBy.ID,
        "com.ariesbifold:id/ViewProofRecord",
    )
    view_revocation_notification_button_locator = (
        AppiumBy.ID,
        "com.ariesbifold:id/ViewRevocation",
    )
    view_revocation_notification_button_aid_locator = (
    ...
    def select_credential_offer_notification(self):
        if super().on_this_page(self.on_this_page_notification_locator):
            self.find_by(
                self.view_credential_offer_notification_button_locator,
    ...
    def select_revocation_notification(self):
        if super().on_this_page(
            self.on_this_page_revocation_notification_locator, timeout=20
        ):
            ...
            # else:
            self.find_by(
                self.view_revocation_notification_button_locator,
                wait_condition=WaitCondition.ELEMENT_TO_BE_CLICKABLE,
            ).click()
