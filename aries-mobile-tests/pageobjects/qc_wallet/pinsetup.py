...
class PINSetupPageQC(PINSetupPage):
    ...
    def get_error(self):
        # On Android the modal hides all the other PIN setup page elements, so we can't check on this page
        # if self.on_this_page():        
        # logging.info(f"modal_message_body_locator2: {self.modal_message_body_locator}")
        logging.info(f"error_message_pin_locator : {self.modal_message_body_locator}")
        logging.info(f"error_message_pin_locator : {self.error_message_pin_locator}")
