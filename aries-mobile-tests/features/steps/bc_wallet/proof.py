...
from agent_test_utils import (create_non_revoke_interval,
                              get_qr_code_from_invitation, table_to_str)
...
from pageobjects.bc_wallet.information_sent_successfully import \
    InformationSentSuccessfullyPage
...
def step_impl(context):
    context.thisAreYouSureDeclineProofRequest = (
...
def step_impl(context):
    who, attributes, values = get_expected_proof_request_detail(context)
...
def step_impl(context):
    ...
    credential_card_text_list = (
        context.thisProofRequestPage.get_text_in_all_credential_cards_in_proof_request()
    )
...
@when("the user has a proof request for {proof}")
@when(
    "the user has a proof request for {proof} including proof of non-revocation at {interval}"
)
...
def step_impl(context, credential_name):
    ...
    credential_details = (
        context.thisProofRequestDetailsPage.get_first_credential_details()
    )
...
def step_impl(context):
    context.thisSendingInformationSecurleyPage = (
        context.thisProofRequestPage.select_share()
    )
...
def step_impl(context):
    context.thisHomePage = (
        context.thisInformationSentSuccessfullyPage.select_back_to_home()
    )
...
def step_impl(context):
    ...
    if (
        "autoGrantPermissions" in context.driver.capabilities
        and context.driver.capabilities["autoGrantPermissions"] == False
    ) or (context.driver.capabilities["platformName"] == "iOS"):
    ...
    if context.thisQRCodeScanPage.on_this_page():
        ...
        if "Invalid QR code" in context.thisQRCodeScanPage.get_page_source():
            ...
            logging.info(
                "There seems to be a problem scanning the QR Code, closing and scanning again"
            )
        if context.thisQRCodeScanPage.on_this_page():
...
def step_impl(context, proof):
    ...
    qrcode = context.verifier.send_proof_request(
        request_for_proof=proof_json, connectionless=True
    )
...
def step_impl(context):
    for row in context.table:
        credential_name = row["credential_name"]
        context.issuer.revoke_credential(notify_holder=True, credential=credential_name)


@then("the proof is unverified")
def step_impl(context):
    raise NotImplementedError("STEP: Then the proof is unverified")
...
def step_impl(context):
    context.thisCredentialDetailsPage = (
        context.thisHomePage.select_revocation_notification()
    )
    context.thisCredentialDetailsPage.on_this_page()


@then("the holder reviews the contents of the revocation notification message")
def step_impl(context):
...
@then("the revocation notification is removed")
def step_impl(context):
...
@given("The holder has received and acknowledged the revocation message notification")
def step_impl(context):
    context.execute_steps(
        f"""
        Then the holder receives a revocation message notification
...
def step_impl(context):
    ...
    context.thisCredentialDetailsPage = (
        context.thisCredentialsPage.select_top_credential()
    )
...
@then("the holder will be able to review the revoked message again")
def step_impl(context):
    context.thisCredentialDetailsPage.select_revocation_info()
    assert (
        context.thisCredentialDetailsPage.get_revocation_message()
