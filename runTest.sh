...
LEDGER_URL_CONFIG=http://test.bcovrin.vonx.io REGION=us-west-1 ./manage run -d SauceLabs -u $DEVICE_CLOUD_USERNAME -k $DEVICE_CLOUD_ACCESS_KEY -p Android -a $APP_NAME.apk -i acapy-main -v acapy-main -t @DID-372
echo -e '\n=> Done'

#Notes:
# regardin nehave the @is what is required to run the test. If you want to run everything use @ODS otherwise just the test
